...
(() => {
  ...
  // Main Functionality
  const drawAnchor = () => {
    // get the position for the anchor on the canvas - right behind the avatar
  ...
  const attachEventListeners = () => {
    // listen to the mousemove event and set mouse coordinates
  ...
  const loop = () => {
    requestAnimationFrame(loop); // redraw every frame
    drawAnchor();
